...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/huutvpn/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/huutvpn/backupuserhuutvpn.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "abuddin388@gmail.com" &> /dev/null
    git config --global user.name "huutvpn" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/huutvpn/backupuserhuutvpn
    git push -f https://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@github.com/huutvpn/backupuserhuutvpn.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/huutvpn/backupuserhuutvpn/main/$NameUser/$NameUser.zip"
sleep 1
