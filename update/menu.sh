...
export COLOR1="$(cat /etc/huutvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/huutvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
echo "IP=$host" > /var/lib/huutvpn/ipvps.conf
echo ""
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/huutvpn/update/main/install_up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/huutvpn/update/main/version_up)
...
serverV=$( curl -sS https://raw.githubusercontent.com/huutvpn/update/main/version_up)
if [ "$Isadmin" = "ON" ]; then
