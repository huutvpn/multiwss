...
colornow=$(cat /etc/huutvpn/theme/color.conf)
NC="\e[0m"
COLOR1="$(cat /etc/huutvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/huutvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"

APIGIT=$(cat /etc/huutvpn/github/api)
EMAILGIT=$(cat /etc/huutvpn/github/email)
USERGIT=$(cat /etc/huutvpn/github/username)
...
if [[ -f /etc/yokkovpn/github/api && -f /etc/huutvpn/github/email && /etc/huutvpn/github/username ]]; then
   rec="OK"
else
    mkdir /etc/huutvpn/github > /dev/null 2>&1
fi
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • huutvpn.huutvpn •            $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e ""
...
sleep 2
echo "$EMAIL1" > /etc/huutvpn/github/email
echo "$USERNAME1" > /etc/huutvpn/github/username
echo "$API1" > /etc/huutvpn/github/api
echo "ON" > /etc/huutvpn/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/huutvpn/github/email
rm -f /etc/huutvpn/github/username
rm -f /etc/huutvpn/github/api
rm -f /etc/huutvpn/github/gitstat
echo "OFF" > /etc/huutvpn/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/huutvpn/permission/main/ipmini | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
echo -e "$COLOR1│${NC} [INFO] PM : huutvpn huutvpn huutvpn / wa.me/6283877826797/"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • huutvpn.huutvpn •            $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
GITREQ=/etc/huutvpn/github/gitstat
if [ -f "$GITREQ" ]; then
    cekk="ok"
else 
    mkdir /etc/huutvpn/github
    touch /etc/huutvpn/github/gitstat
    echo "OFF" > /etc/huutvpn/github/gitstat
fi

stst1=$(cat /etc/huutvpn/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1│${NC} ${COLBG1}               • REGISTER IPVPS •              ${NC} $COLOR1│$NC"
...
stst=$(cat /etc/huutvpn/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
